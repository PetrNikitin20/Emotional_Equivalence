"""
Analysis of cultural markers and the cultural-density profile
"""
...
class CulturalDensityAnalyzer:
    ...
    def create_plot(self, profile):
        if not profile['density_profile']:
            return "<div>No cultural markers detected</div>"
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate='Position: %{x}<br>Density: %{y:.1f}<extra></extra>'
        ))

        fig.add_hline(y=profile['avg_density'], line_dash="dash", line_color="orange",
                      annotation_text=f"Mean: {profile['avg_density']}")
        ...
        fig.update_layout(
            ...
            yaxis_title="Density (markers per 100 words)",
            hovermode='closest',
        ...
        fig.add_annotation(
            ...
            x=0.02, y=0.98,
            text=f"Total unique markers: {profile['total_markers']} | Peak density: {profile['max_density']}",
            showarrow=False,
            bgcolor='white',
            bordercolor='#667eea',
            borderwidth=1
