"""
Extraction of cultural allusions (model: Qwen2.5:7b)
"""
...
if __name__ == "__main__":
    ...
    for filename, bookname in my_books.items():
        ...
        for i, chunk in enumerate(chunks):
            ...
            print(f"Found: {len(allusions)} allusions")
            
            book_allusions.extend(allusions)
            
            time.sleep(1)
        
        print(f"\nTotal in {bookname}: {len(book_allusions)} allusions")
    ...
    if all_allusions:
        ...
        print(f"Total unique allusions found: {len(unique)}")
